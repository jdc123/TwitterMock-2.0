...
and if so it simply resends the response.*/	
	...
	//if Primary 
	//if(isPrimary){ //Not necessarily: P sends updates to B1, not B2. B2 becomes the primary
		//if Updates found in log, resend them
		//No reads are logged
		if((strncmp(allTokens[0], "REGISTER", strlen(allTokens[0])) == 0) ||
			...
			(strncmp(allTokens[0], "DELETE_ACCOUNT", strlen(allTokens[0])) == 0)){
				...
				//int found = checkLog(request, response); //will store only updates
				//char response[MAX_RESPONSE_SIZE]; //change global response size
				int found = 0;
				if(found){
					if(send(clientSock, response, MAX_RESPONSE_SIZE, 0) == -1) //Sending to FE so no need to check for timeouts
					{
						perror("Send Failed.");
						exit(SEND_FAILED);
					}
